...
// Canais ordenados: Home primeiro, resto alfabético
...
export default function AnalytrickApp() {
  ...
  function getOrderBy(tab: TabId) {
    const map: Record<TabId, { col: string; asc: boolean }> = {
    ...
    return map[tab] || { col: 'id', asc: false }
  }
  ...
  const HEADER_COLOR: Record<TabId, { hc: string; hcs: string }> = {
  ...
  // Ajusta cor do header de anúncios e vendedores pelo canal
  const CHANNEL_COLORS: Record<string, { anuncios: string; vendedores: string }> = {
  ...
  function getHeaderColor(tab: TabId) {
    const chColors = CHANNEL_COLORS[chKey] || CHANNEL_COLORS.ml
    if (tab === 'anuncios')   return { hc: chColors.anuncios,  hcs: chColors.anuncios  + 'cc' }
    if (tab === 'vendedores') return { hc: chColors.vendedores, hcs: chColors.vendedores + 'cc' }
    return HEADER_COLOR[tab]
  }
  ...
  const btnH1 = (active = false, red = false): React.CSSProperties => ({
    ...
    background: red   ? 'rgba(220,38,38,.7)'
               : active ? '#ffe600'
               : 'rgba(255,255,255,.1)',
    color:  red ? '#fff' : active ? '#111' : 'rgba(255,255,255,.8)',
    border: red   ? '1px solid rgba(185,28,28,.8)'
               : active ? '1px solid #d4b800'
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0, marginRight: 8 }}>
          ...
          <span style={{ fontSize: 10, fontWeight: 400, color: '#ffe600', opacity: .8 }}>
        ...
        {/* Espaço flex vazio — empurra tabs e ações para direita */}
        <div style={{ flex: 1 }} />

        {/* ── Tabs módulos — direita ── */}
          ...
          <div style={{ position: 'relative' }} ref={configRef}>
            ...
            {showConfigPanel && (
              <div style={{
                ...
                width: 280, boxShadow: '0 8px 40px rgba(0,0,0,.6)',
                ...
                <div style={{ padding: 16, borderBottom: `1px solid ${brd}` }}>
                  ...
                  <div style={{ fontSize: 9, color: txtVD, marginBottom: 8 }}>% Imposto aplicado sobre o PREÇO de cada item</div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2, flex: 1, minWidth: 0, overflow: 'hidden' }}>
          ...
          <span style={{ fontSize: 9, color: txtVD, opacity: .7 }}>
            💡 ⚙ Configurações · % Imposto = aplica sobre o preço
          </span>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
          ...
          {isAdmin && (
            <button style={footerBtn} title="Ver logs">📋 LOG</button>
          )}
        </div>
      </div>
