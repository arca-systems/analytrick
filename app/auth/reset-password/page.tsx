...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    if (hash && hash.includes('type=recovery')) {
      ...
      supabase.auth.getSession().then(({ data }) => {
        ...
        else setTimeout(() => setReady(true), 2000)
      })
    }
  }, [])
  ...
  async function handleReset(e: React.FormEvent) {
    ...
    else { setSuccess(true); setTimeout(() => router.push('/app'), 2000) }
  }
        ...
        {success ? (
          ...
          <form onSubmit={handleReset} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div>
              ...
              <input type="password" required value={confirm} onChange={e => setConfirm(e.target.value)} placeholder="Repita a nova senha" style={inputStyle}
                onFocus={e => e.target.style.borderColor = '#3b82f6'} onBlur={e => e.target.style.borderColor = '#374151'} />
            </div>
            {error && (
            ...
            <button type="submit" disabled={loading} style={{
              background: '#2563eb', border: 'none', borderRadius: 8, color: '#fff', fontSize: 13,
